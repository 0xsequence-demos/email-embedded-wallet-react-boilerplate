...
// This value should be stored in your .env file, not hardcoded here
const BUILDER_PROJECT_ID = "";
...
export const onRequestVerifyJWT = async ({sequenceToken} : {sequenceToken?: string}) => {
  ...
  try {
    const verified = await verifyJWT(
      sequenceToken,
      `https://sequence.build/project/${BUILDER_PROJECT_ID}`,
    );
